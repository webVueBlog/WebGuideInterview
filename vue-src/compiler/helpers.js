...
export function addHandler (
  ...
  if (modifiers.right) {
	...
	// 如果设置了middle修饰符，则自动将click事件重置为mouseup，因为浏览器并没有提供滚轮click事件，只能用mouseup触发。通过mouseup的event.button可以区分按下的是鼠标的哪一个按键
    // 中间键
    if (dynamic) {
  ...
  if (Array.isArray(handlers)) {
	  // 无论有多少个同名事件的监听，都不会落下任何一个监听函数的执行，因此，可以在同一个元素下绑定多个同名事件
...
export function getBindingAttr (
  ...
  if (dynamicValue != null) {
	...
	// 如果存在staticValue(即普通字符串，而不是对应着某个变量)，则将其做stringify处理(防止在new Function的时候被转换成变量)
    if (staticValue != null) {
