...
export function genAssignmentCode (
  ...
  // 如果 res 存在key，即 v-model="information.name"或者v-model="information[name]"
  if (res.key === null) {
	// 将assignment(即控件绑定的value)赋值给value（表达式中的value，上例子中的information）
...
export function parseModel (val: string): ModelParseResult {
  ...
  if (val.indexOf('[') < 0 || val.lastIndexOf(']') < len - 1) {
	...
	// 将val转换为对象形式，以上述为例：
	// {exp: 'information', key: 'name.shortName'}
    if (index > -1) {
      return {
  ...
  // 最终处理为 {exp:'information', key:'name'}
  // 但是如果使用了多个[]，如v-model="information[name][firstName]"
  // 最终key为最后一个最外层的[]
  // {exp:'information[name]', key:'firstName'}
  return {
