...
export function parseHTML(html, options) {
  const stack = [] // 处理标签不闭合问题，利用解括号闭合的思想( [{({)}] )入栈出栈
  ...
  while (html) {
    ...
    if (!lastTag || !isPlainTextElement(lastTag)) {
      ...
      if (textEnd === 0) {
        ...
        if (comment.test(html)) {
          ...
          // 注释标签的结束索引
          const commentEnd = html.indexOf('-->') //注释节点必须以 --> 结尾
	...
	// 当结尾的text为 0<1<2 的时候，最后的 <2 无法处理，因为
    ...
    if (html === last) {
	  ...
	  // 进入此判断的必要条件，就是末尾没有标签，若此时stack还存在标签，则代表此文本内容出现在了标签以外
  ...
  // 循环完毕时，检测stack中是否还存在标签，若有则代表有不闭合标签，利用parseEndTag 发出警告
